...
import useSearchGoogle from "@/hooks/use-search-google";
...
const Home = () => {
  ...
  const {products, loading, error, linkLoading} = useSearchGoogle(query, sortBy);
  return (
    <div className="grid grid-rows-[120px_100px_1fr] items-center justify-items-center min-h-screen p-8 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      ...
      <div className="w-full flex flex-col gap-4">
        ...
        {products && Array.isArray(products) && products.length > 0 && products.map((product, index) => {
          return <ProductCard product={product} key={index} linkLoading={linkLoading} />;
        })}
